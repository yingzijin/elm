...
async function lottery(cookie) {
  ...
  const mh5tk = getToken(cookie);
  const _mh5tk = mh5tk["split"]('_')[0];
  const sign = md5(_mh5tk + '&' + t + '&' + '12574478' + '&' + JSON["stringify"](data));
  const config = {
    "url": "https://guide-acs.m.taobao.com/h5/mtop.koubei.interactioncenter.platform.right.lottery/1.0/?jsv=2.6.1&appKey=12574478&t=" + t + "&sign=" + sign + "&api=mtop.koubei.interactioncenter.platform.right.lottery&v=1.0&type=originaljson&dataType=json",
    "method": "POST",
    "headers": linkgame_url,
    "body": body
  };
  return tryCatchPromise(flag => {
    request(config, async (_0x53422a, _0x24898a, _0xb8dfb5) => {
      if (!_0x53422a && _0x24898a["statusCode"] === 200) {
        try {
          const json_body = JSON["parse"](_0xb8dfb5);
        //   gametime = decodeURIComponent(json_body['data']['url']).split('t=')[1].split('&sign=')[0];
          if (isEmpty(json_body["data"]["data"])) {
            console["log"](json_body["ret"][0]);
            flag(false);
          } else {
            if (json_body["data"]["data"]["errorMsg"]) {
              console["log"](json_body["data"]["data"]["errorMsg"]);
            } else {
              let _0x14c287 = json_body["data"]["data"]["sendRightList"][0];
...
async function lyb_sign(h5_cookie) {
  const cookie = await checkCk(h5_cookie);
  const header = {
  ...
  const data = {
    "bizScene": "game_center",
    "asac": "2A21607NIIT1ND5C4YXJ6C",
    "umidtoken": "defaultToken2_load_failed with timeout@@https://tb.ele.me/wow/alsc/mod/b9ee9e6451bc8eda7a6afcbb@@" + t
  };
  var body = "data=" + encodeURIComponent(JSON["stringify"](data));
  const mh5tk = getToken(cookie);
  const _mh5tk = mh5tk["split"]('_')[0];
  const sign = md5(_mh5tk + '&' + t + '&' + '12574478' + '&' + JSON["stringify"](data));
  const config = {
    "url": "https://mtop.ele.me/h5/mtop.koubei.interactioncenter.sign.component.recordsignin/1.0/5.0/?jsv=2.7.1&appKey=12574478&t=" + t + "&sign=" + sign + "&api=mtop.koubei.interactioncenter.sign.component.recordsignin&v=1.0&ecode=1&type=json&valueType=string&needLogin=true&LoginRequest=true&dataType=jsonp&SV=5.0&asac=2A21607NIIT1ND5C4YXJ6C&secttid=h5%40android_chrome_87.0.4280.141",
    "method": "POST",
    "headers": header,
    "body": body
  };
  return tryCatchPromise(_0x35144b => {
    request(config, async (_0x2a14dd, _0x1b1f47, _0x2f2f86) => {
...
async function lyb_llk_token(h5_cookie) {
  const header = {
  ...
  const body = await gameRequest(h5_cookie, header);
  return [body["data"]["token"], body["data"]["openId"]];
}
async function lyb_llk_gamecode(h5_cookie, token) {
  const header = {
    "bizScene": "LIANLIANKAN",
    "bizMethod": "startGameV2",
    "bizParam": "{\"gameId\":null,\"token\":\"" + token + "\"}",
    "longitude": 114.174328,
    "latitude": 22.316555
  };
  const game_body = await gameRequest(h5_cookie, header);
  if (game_body["bizErrorMsg"] != "success") {
    console["log"](game_body["bizErrorMsg"]);
    return null;
  }
  return game_body["data"]["gameCode"];
}

async function lyb_llk_passgame(h5_cookie, gamecode, token, openId) {
  const sign = md5(`Game[${openId}]-${token}|${gamecode}${gametime}`);
//   console.log(sign)
  const plat = {
    "bizScene": "LIANLIANKAN",
    "bizMethod": "settlement",
    "bizParam": "{\"gameCode\":\"" + gamecode + "\",\"passLevelTime\":" + gametime + ",\"sign\":\"" + sign + "\",\"gameId\":null,\"token\":\"" + token + "\"}"
  };
   const result = await gameRequest(h5_cookie, plat);
//    console.log(result)
...
async function gameRequest(cookie, plat) {
  const header = {
    "authority": "shopping.ele.me",
    "accept": "application/json",
    "accept-language": "zh-CN,zh;q=0.9,en-US;q=0.8,en;q=0.7",
  ...
  const t = new Date()["getTime"]();
  var body = "data=" + encodeURIComponent(JSON["stringify"](plat));
  const mh5tk = getToken(cookie);
  const _mh5tk = mh5tk["split"]('_')[0];
  const sign = md5(_mh5tk + '&' + t + '&' + '12574478' + '&' + JSON["stringify"](plat));
  const config = {
  ...
  return tryCatchPromise(_0x21767f => {
    request(config, async (statusCode, _0x9ba4ae, body) => {
      if (!statusCode && _0x9ba4ae["statusCode"] == 200) {
        try {
          const json_body = JSON["parse"](body);
          const _0x6b9773 = JSON["parse"](json_body["data"]["data"]);
          _0x21767f(_0x6b9773);
        } catch (_0x197b21) {
          console["log"](body);
          _0x21767f(null);
...
async function llk_game(h5_cookie, token, openId) {
  const gamecode = await lyb_llk_gamecode(h5_cookie, token);
  const sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));
  if (gamecode) {
    await sleep (gametime);
    const lastLevelId = await lyb_llk_passgame(h5_cookie, gamecode, token, openId);
    if (lastLevelId != 3) {
      console.log(`美味天天连第${lastLevelId}轮`);
      await llk_game(h5_cookie, token, openId);
    }
    if (lastLevelId != "null" && lastLevelId == 3) {
        console.log("美味天天连完成");
    }
  }
  return;
...
async function start() {
  ...
  for (let i = 0; i < cookie_list["length"]; i++) {
    ...
    if (!cookie) {
      ...
      try {
        let h5_cookie = await checkCk(cookie, i);
        if (!h5_cookie) {
          continue;
        }
        let user_info = await getUserInfo(h5_cookie);
        if (!user_info["username"]) {
          console["log"]('第', i + 1, "账号失效！请重新登录！！！😭");
          continue;
        }
        const userId = user_info["user_id"];
        console["log"]("******开始【账号", i + 1, '】', userId, user_info["username"], "*********");
        // 乐园币签到
        await lyb_sign(h5_cookie);
        // 获取token
        await lottery(h5_cookie);
        const [token, openId] = await lyb_llk_token(h5_cookie);
        // 连连看游戏
        await llk_game(h5_cookie, token, openId);
        // 欢乐倒水游戏
        await water_game(h5_cookie);
